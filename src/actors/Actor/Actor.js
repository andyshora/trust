...
const MATERIALS = {
  dove: new MeshStandardMaterial({
    color: 0xFFFFFF,
    wireframe: true,
    roughness: 0.18,
    metalness: 0
  }),
  hawk: new MeshStandardMaterial({
    color: 0xFF0000,
    wireframe: true,
    roughness: 0.18,
    metalness: 0
...
class Actor extends Component {
  ...
  _loadMeshes = ({
    ...
    names.forEach((name, i) => {
      ...
      const parentGroup = group.length ? group[i] : group
      loader.load(
      	// resource URL
      	`${name}.obj`,
          ...
          object.children.forEach(mesh => mesh.material = material)
  ...
  _setupObjects() {
    ...
    switch (behaviour) {
      case 'spinning': {
        ...
        this._loadMeshes({ names, group: this._objectGroup })
